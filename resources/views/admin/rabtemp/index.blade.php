...
@section('content')
        ...
        <table id="datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
          ...
          <tbody>
            @foreach ($rab as $key => $value)
              ...
              <tr>
                <td>{{ $key + 1 }}</td>
                <td>{{ count($value->barang) > 0 ? $value->barang[0]->nama : '' }}</td>
                <td>{{ count($value->barang) > 0 ? $value->barang[0]->suplier[0]['nama'] : '' }}</td>
                <td>{{ $value->untuk }}</td>
                <td>@currency($value->harga_barang)</td>
                <td>{{ $value['kuantitas'] }}</td>
                <td>
                  @if (count($value->barang) > 0)
                    @currency($value->barang[0]->harga * $value['kuantitas'])
                  @endif
                </td>
                <td>
                  <a href="#edit-modal" data-animation="sign" data-plugin="custommodal" data-id='{{ $value->id }}'
                    data-nama="{{ $value['nama'] }}" data-overlaySpeed="100" data-overlayColor="#36404a"
                    class="btn btn-success btn-sm modal_edit"><i class="fa fa-edit"></i></a>
                  <a href="#hapus-modal" data-animation="sign" data-plugin="custommodal" data-id='{{ $value->id }}'
                    data-overlaySpeed="100" data-overlayColor="#36404a" class="btn btn-danger btn-sm hapus"><i
  ...
  <div id="tambah-modal" class="modal-demo">
    ...
    <div class="custom-modal-text">
      ...
      <div class="p-20 text-left">
        <form class="form-horizontal m-t-20" enctype="multipart/form-data" action="{{ route('rabtemp.store') }}"
          method="POST">
          {{ csrf_field() }}
  ...
  <div id="hapus-modal" class="modal-demo">
    ...
    <div class="custom-modal-text">
      ...
      <div class="p-20">
        ...
        <form class="form-horizontal m-t-20" enctype="multipart/form-data" action="{{ route('rabtemp.hapus') }}"
          method="POST">
          {{ csrf_field() }}
          <div>
  ...
  <div id="selesai-modal" class="modal-demo">
    ...
    <div class="custom-modal-text">
      ...
      <div class="p-20 text-left">
        <form class="form-horizontal m-t-20" enctype="multipart/form-data" action="{{ route('rabtemp.selesai') }}"
          method="POST">
          {{ csrf_field() }}
